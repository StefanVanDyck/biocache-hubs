...
function init() {
    ...
    $("#relatedRecordReason").on("change", function(e) {
        ...
        if (relatedRecordReason === "") {
            ...
            $(col_reason).text(
                jQuery.i18n.prop(
                    "related.record.reason.description." + relatedRecordReason,
                ),
            );
        }
        setIssueFormButtonState();
    ...
    $("#relatedRecordId").on("change", function(e) {
        ...
        if (val === OCC_REC.recordUuid) {
            ...
            $.get(OCC_REC.contextPath + "/occurrence/exists/" + val)
                .success(function(data) {
                    ...
                    if (data.error) {
                        ...
                        $("#related-record-id-not-found")
                            .text(
                                "More than 1 record found with specified id, please use a more specific id",
                            )
                            .show();
                        // hide compare table
                        $("#records_comparison_heading").hide();
                ...
                .error(function() {
                    $("#related-record-id-not-found")
                        .text("The record id can't be found.")
                        .show();
                    $("#related-record-id-found").hide();
                    $("#related-record-id-loading").hide();
                    $("#records_comparison_table").hide();
    ...
    $("form#issueForm").submit(function(e) {
        ...
        if (code != "") {
            ...
            $.get(
                OCC_REC.contextPath + "/assertions/" + OCC_REC.recordUuid,
                function(data) {
                    ...
                    for (var i = 0; i < data.userAssertions.length; i++) {
                        if (data.userAssertions[i].code == 50000) {
                            data.userAssertions.forEach(function(element) {
                                if (
                                    element.uuid == data.userAssertions[i].relatedUuid &&
                                    element.code == code
                                ) {
                    ...
                    if (bPreventAddingIssue) {
                        alert(
                            "You cannot flag an issue with the same type that has already been verified.",
                        );
                        return;
                    } else if (code == "20020" && !relatedRecordId) {
                        alert(
                            "You must provide a duplicate record id to mark this as a duplicate",
                        );
                        return;
                    } else if (code == "20020" && !relatedRecordReason) {
                        alert(
                            "You must select a reason to mark this record as a duplicate",
                        );
                        return;
                        ...
                        $.post(
                            OCC_REC.contextPath + "/occurrences/assertions/add",
                            {
                            ...
                            function(data) {
                                ...
                                if (OCC_REC.myAnnotationEnabled) {
                                    ...
                                    var actionpath = new_state
                                        ? "/api/subscribeMyAnnotation"
                                        : "/api/unsubscribeMyAnnotation";
                            ...
                            $("#assertionSubmitProgress").css({ display: "none" });
                            $("#submitSuccess").html(
                                jQuery.i18n.prop("show.issueform.flagfail.message"),
                            );
                        });
                    }
                },
    ...
    $("#assertionButton").click(function(e) {
        ...
        if (OCC_REC.myAnnotationEnabled) {
            ...
            $.getJSON(getAlerts, function(data) {
                var myAnnotationEnabled =
                    data && data.myannotation && data.myannotation.length > 0;
        ...
        if (text) {
            $.each(text, function(j, el) {
                ...
                if (sensitiveDatasets[code]) {
                    var linked =
                        "<a href='" +
                        sensitiveDatasets[code] +
                        "' title='" +
                        list +
                        " sensitive species list information page' target='collectory'>" +
                        list +
                        "</a>";
    ...
    $("#backBtn a").click(function(e) {
        ...
        if (url) {
            ...
            alert(
                "Sorry it appears the history has been lost, please use the browser&apso;s back button",
            );
        }
    });
    ...
    if (sequenceStr.length > 10) {
        ...
        $(sequenceTd).html(
            "<code>" + sequenceStr.replace(/(.{10})/g, "$1 ") + "</code>",
        );
    }
    ...
    $(".dataQualityHelpLinkZZZ").click(function(e) {
        ...
        if (dataQualityItem) {
            ...
            content +=
                "<div><a href='https://github.com/AtlasOfLivingAustralia/ala-dataquality/wiki/" +
                dataQualityItem.name +
                "' target='wiki' title='More details on the wiki page'>Wiki page</a></div>";
        }
    ...
    $(".dataQualityHelpLink")
        .popover({
            ...
            title: function() {
                ...
                if (dataQualityItem) {
                    content =
                        "<button type='button' class='close' onclick='$(&quot;.dataQualityHelpLink&quot;).popover(&quot;hide&quot;);'>×</button>" +
                        dataQualityItem.name;
                }
                return content;
            },
            content: function() {
                ...
                if (dataQualityItem) {
                    ...
                    if (dataQualityItem.wiki) {
                        content +=
                            "<div><i class='glyphicon glyphicon-share-alt'></i>&nbsp;<a href='https://github.com/AtlasOfLivingAustralia/ala-dataquality/wiki/" +
                            dataQualityItem.name +
                            "' target='wiki' title='More details on the wiki page'>Wiki page</a></div>";
                    }
                }
    ...
    $("#copy-al4r").on("click", function() {
        ...
        if (navigator.clipboard && window.isSecureContext) {
            ...
            navigator.clipboard
                ...
                .then(() => {
                    alert(jQuery.i18n.prop("list.copylinks.tooltip.copied"));
                })
                .catch((error) => {
                    alert(jQuery.i18n.prop("list.copylinks.alert.failed") + error);
                });
        } else {
            alert(
                "Copying to clipboard requires a secure HTTPS connection. Value: " +
                input.value,
            );
        }
    });
    ...
    $("#loginOrFlag").on("show.bs.modal", function(event) {
        ...
        if (editMode === "true") {
            ...
            $("#loginOrFlag")
                .find("#loginOrFlagLabel")
                .text(jQuery.i18n.prop("show.loginorflag.title.edit"));
            ...
            $("#loginOrFlag")
                .find("#loginOrFlagLabel")
                .text(jQuery.i18n.prop("show.loginorflag.title"));
...
function deleteAssertion(recordUuid, assertionUuid) {
    $.post(
        OCC_REC.contextPath + "/occurrences/assertions/delete",
...
function refreshUserAnnotations() {
    $.get(
        ...
        function(data) {
            if (
                data.assertionQueries.length == 0 &&
                data.userAssertions.length == 0
            ) {
            ...
            for (var i = 0; i < data.assertionQueries.length; i++) {
                ...
                if (data.assertionQueries[i].hasOwnProperty("comment")) {
                    $clone
                        .find(".comment")
                        .text("Comment: " + data.assertionQueries[i].comment);
                }
                $clone
                    .find(".created")
                    .text(
                        "Date created: " +
                        moment(data.assertionQueries[i].createdDate).format("YYYY-MM-DD"),
                    );
                if (data.assertionQueries[i].recordCount > 1) {
                    ...
                    $clone
                        ...
                        .attr(
                            "href",
                            OCC_REC.contextPath +
                            "/occurrences/search?q=query_assertion_uuid:" +
                            data.assertionQueries[i].uuid,
                        );
                }
            ...
            $.each(data.userAssertions, function(index, userAssertion) {
                ...
                if (userAssertion.code != 50000) {
                    ...
                    $clone
                        .find(".issue")
                        .text(jQuery.i18n.prop(userAssertion.name))
                        .attr("i18nkey", userAssertion.name);
                    ...
                    $clone
                        .find(".userRole")
                        .text(userAssertion.userRole != null ? userAssertion.userRole : "");
                    $clone
                        .find(".userEntity")
                        .text(
                            userAssertion.userEntityName != null
                                ? userAssertion.userEntityName
                                : "",
                        );
                    $clone
                        .find(".created")
                        .text(
                            "Date created: " +
                            moment(userAssertion.created, "YYYY-MM-DDTHH:mm:ssZ").format(
                                "YYYY-MM-DD HH:mm:ss",
                            ),
                        );
                    if (userAssertion.relatedRecordId) {
                        ...
                        $clone
                            .find(".related-record-id-span")
                            .text(userAssertion.relatedRecordId);
                        var href = $clone.find(".related-record-link").attr("href");
                        $clone
                            .find(".related-record-link")
                            .attr(
                                "href",
                                href.replace("replace-me", userAssertion.relatedRecordId),
                            );
                        if (userAssertion.code === 20020) {
                            ...
                            $.get(
                                OCC_REC.contextPath +
                                "/occurrence/exists/" +
                                userAssertion.relatedRecordId,
                            ).success(function(data) {
                                if (!data.error) {
                                    if (data.scientificName) {
                                        $clone.find(".related-record-name").show();
                                        $clone
                                            .find(".related-record-name-span")
                                    ...
                                    if (data.stateProvince) {
                                        ...
                                        $clone
                                            .find(".related-record-state-span")
                                            .text(data.stateProvince);
                                    }

                                    if (data.decimalLongitude) {
                                        $clone.find(".related-record-latitude").show();
                                        $clone
                                            .find(".related-record-latitude-span")
                                            .text(data.decimalLongitude);
                                    }

                                    if (data.decimalLatitude) {
                                        $clone.find(".related-record-longitude").show();
                                        $clone
                                            .find(".related-record-longitude-span")
                                            .text(data.decimalLatitude);
                                    }

                                    if (data.eventDate) {
                                        $clone.find(".related-record-eventdate").show();
                                        $clone
                                            .find(".related-record-eventdate-span")
                                            .text(data.eventDate);
                                    }
                                }
                            });
                    ...
                    if (userAssertion.relatedRecordReason) {
                        ...
                        $clone
                            .find(".related-record-reason-span")
                            .text(
                                jQuery.i18n.prop(
                                    "related.record.reason." + userAssertion.relatedRecordReason,
                                ),
                            )
                            .attr(
                                "i18nkey",
                        ...
                        $clone
                            .find(".related-record-reason-explanation")
                            .text(
                                jQuery.i18n.prop(
                                    "related.record.reason.explanation." +
                                    userAssertion.relatedRecordReason,
                                ),
                            )
                            .attr(
                                "i18nkey",
                    ...
                    if (userAssertion.userEntityName != null) {
                        $clone
                            .find(".userEntity")
                            .text(", " + userAssertion.userEntityName);
                    }
                    ...
                    $clone
                        .find(".verifyAnnotation")
                        .attr("id", "verifyAnnotations_" + userAssertion.uuid);
            ...
            //display verified
            var sortedVerifiedAssertion =
                verifiedAssertions.sort(compareModifiedDate);
            for (var i = 0; i < sortedVerifiedAssertion.length; i++) {
                var $clone = $("#userVerificationTemplate").clone();
                $clone.prop(
                    "id",
                    "userVerificationAnnotation_" + sortedVerifiedAssertion[i].uuid,
                );
                var qaStatusMessage = jQuery.i18n.prop(
                    "user_assertions." + sortedVerifiedAssertion[i].qaStatus,
                );
                $clone
                    .find(".qaStatus")
                ...
                $clone
                    .find(".userDisplayName")
                    .text(sortedVerifiedAssertion[i].userDisplayName);
                $clone
                    .find(".created")
                    .text(
                        moment(
                            sortedVerifiedAssertion[i].created,
                ...
                $("#userAnnotationsList")
                    .find(
                        "#userAnnotation_" +
                        sortedVerifiedAssertion[i].relatedUuid +
                        " table.verifications tbody",
                    )
                    .append($clone);
                $("#userAnnotationsList")
                    .find(
            ...
            for (var i = 0; i < disableDelete.length; i++) {
                ...
                $cloneHeader.prop(
                    "id",
                    "userVerificationAnnotationHeader_" + disableDelete[i],
                );
                ...
                $cloneHeader
                    .find(".deleteVerification")
                    .html("Delete this Verification");
                $cloneHeader.css({ display: "block" });
                $cloneHeader.insertAfter(
                    "#userAnnotation_" +
                    disableDelete[i] +
                    " .userVerificationClass .userVerificationTemplate:first",
                );
            }

            for (var i = 0; i < data.userAssertions.length; i++) {
                if (
                    data.userAssertions[i].code != 50000 &&
                    disableDelete.indexOf(data.userAssertions[i].uuid) < 0
                ) {
...
function updateDeleteVerificationEvents(relatedAssertionId) {
    $("#userAnnotation_" + relatedAssertionId + " .deleteVerificationButton").off(
        "click",
    );
    $("#userAnnotation_" + relatedAssertionId + " .deleteVerificationButton").on(
        "click",
        function(e) {
            e.preventDefault();
            ...
            if (isConfirmed === true) {
                deleteAssertion(
                    OCC_REC.recordUuid,
                    this.parentElement.parentElement.id.split("_").pop(),
                );
            }
        },
    );

    $("#userAnnotation_" + relatedAssertionId + " .editVerificationButton").off(
        "click",
    );
    $("#userAnnotation_" + relatedAssertionId + " .editVerificationButton").on(
        "click",
        function(e) {
            e.preventDefault();
            ...
            $(".verifyDone").hide();
            $("#verifySpinner").hide();
            updateConfirmVerificationEvents(
                OCC_REC.recordUuid,
                relatedAssertionId,
                OCC_REC.userDisplayName,
                assertionId,
            );
...
function deleteAssertionPrompt(event) {
    var isConfirmed = confirm(
        "Are you sure you want to delete this flagged issue?",
    );
    if (isConfirmed === true) {
        $("#" + event.data.qa_uuid + " .deleteAssertionSubmitProgress").css({
            display: "inline",
        });
...
function updateEditDeleteEvents(enableDelete, disableDelete) {
    for (var i = 0; i < enableDelete.length; i++) {
        $("#userAnnotation_" + enableDelete[i] + " .deleteAnnotationButton").off(
            "click",
        );
        $("#userAnnotation_" + enableDelete[i] + " .deleteAnnotationButton").click(
            { rec_uuid: OCC_REC.recordUuid, qa_uuid: enableDelete[i] },
            deleteAssertionPrompt,
        );

        $("#userAnnotation_" + enableDelete[i] + " .editAnnotationButton").off(
            "click",
        );
        $("#userAnnotation_" + enableDelete[i] + " .editAnnotationButton").attr(
            "annotationId",
            enableDelete[i],
        );
        $("#userAnnotation_" + enableDelete[i] + " .editAnnotationButton").on(
            "click",
            function(e) {
                e.preventDefault();
                ...
                var comment = $(
                    "#userAnnotation_" + annotationId + " .issueComment",
                ).text();
                $("#loginOrFlag").find("#issue").val(code);
    ...
    for (var i = 0; i < disableDelete.length; i++) {
        $("#userAnnotation_" + disableDelete[i] + " .editAnnotationButton").attr(
            "disabled",
            "disabled",
        );
        $("#userAnnotation_" + disableDelete[i] + " .editAnnotationButton").attr(
            "title",
            "Unable to edit, as this assertion has a verification",
        );
        $("#userAnnotation_" + disableDelete[i] + " .editAnnotationButton").off(
            "click",
        );
        $("#userAnnotation_" + disableDelete[i] + " .editAnnotationButton").on(
            "click",
            function(e) {
                e.preventDefault();
            },
        ...
        $("#userAnnotation_" + disableDelete[i] + " .deleteAnnotationButton").attr(
            "disabled",
            "disabled",
        );
        $("#userAnnotation_" + disableDelete[i] + " .deleteAnnotationButton").attr(
            "title",
            "Unable to delete, as this assertion has a verification",
        );
        $("#userAnnotation_" + disableDelete[i] + " .deleteAnnotationButton").off(
            "click",
        );
        $("#userAnnotation_" + disableDelete[i] + " .deleteAnnotationButton").on(
            "click",
            function(e) {
                e.preventDefault();
            },
...
function updateVerificationEvents(assertionId) {
    ...
    $("#userAnnotation_" + assertionId + " .verifyAnnotation").on(
        "click",
        function(e) {
            e.preventDefault();
