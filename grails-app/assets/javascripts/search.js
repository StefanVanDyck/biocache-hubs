...
function init() {
  ...
  if (location.hash !== "") {
    $('.nav-tabs a[href="' + location.hash.replace("tab_", "") + '"]').tab(
      "show",
    );
  ...
  $("input.facetOpts").change(function() {
    ...
    if (
      BC_CONF.maxFacets &&
      BC_CONF.maxFacets > 0 &&
      selectedFacets > BC_CONF.maxFacets
    ) {
      $("#facetConfigErrors")
        .html(
          jQuery.i18n.prop(
  ...
  $("#updateFacetOptions").click(function(e) {
    ...
    if (
      ...
      selectedFacets.length <= BC_CONF.maxFacets
    ) {
  ...
  if (userFacets) {
    ...
    for (i in facetList) {
      if (typeof facetList[i] === "string") {
        ...
        $(":input.facetOpts[value='" + thisFacet + "']").attr(
          "checked",
          "checked",
        );
      }
  ...
  $("span.lsid")
    ...
    .each(function(i, el) {
      ...
      var jsonUri =
        ...
        lsid +
        "&" +
        BC_CONF.facetQueries +
        "&facets=raw_taxon_name&pageSize=0&flimit=" +
        maxFacets +
        queryContextParam;
      ...
      $.getJSON(jsonUri, function(data) {
          ...
          if (el.fieldName == "raw_taxon_name") {
            $.each(el.fieldResult, function(j, el1) {
              ...
              synList1 +=
                ...
                "_" +
                j +
                "' class='rawTaxonCheckBox' value='" +
                el1.label +
                "'/>&nbsp;" +
                '<a href="' +
                BC_CONF.contextPath +
                "/occurrences/search?q=raw_taxon_name:%22" +
                encodeURIComponent(el1.label) +
                '%22">' +
                el1.label +
                "</a> (" +
                el1.count +
        ...
        if (synListSize >= maxFacets) {
          synList1 +=
            "<div><br>Only showing the first " +
            maxFacets +
            ' names<br>See the "Scientific name (unprocessed)" section in the "Refine results" column on the left for a complete list</div>';
        }
        ...
        $.each(fqs, function(idx, el) {
          ...
          if (vals.length > 0) {
            ...
            if (
              translation &&
              val in translation &&
              typeof translation[val] === "object"
            ) {
              wiki =
                "<a href='https://github.com/AtlasOfLivingAustralia/ala-dataquality/wiki/" +
  ...
  $(".DQCategoryDetailsLink").click(function() {
    ...
    $("#fqdetail-heading-description").text(
      $(link).data("dqcategorydescription"),
    );
    $("#DQDetailsModal .modal-body #filter-value").html(
      "<b>Filter applied: </b><i>fq=" + fq + "</i>",
    );
    $("#view-excluded").attr("href", dqInverse);

    if (excludeCounts[dqCategoryLabel] !== "0") {
      $("#excluded .exclude-count-label")
        .text(excludeCounts[dqCategoryLabel])
        .removeData("category")
        .removeAttr("category");
    } else {
      $("#excluded .exclude-count-label")
        .text(excludeCounts[dqCategoryLabel])
        .data("category", dqCategoryLabel)
        .attr("data-category", dqCategoryLabel);
    }
    ...
    $.when.apply($, requests).done(function() {
      if (numberOfResponse === 1) {
        if (successStatus === arguments[1] && arguments[0].length > 0) {
          map.set(arguments[0][0].name, [
            arguments[0][0].info
              ? arguments[0][0].info
              : arguments[0][0].description
                ? arguments[0][0].description
                : "",
            arguments[0][0].infoUrl ? arguments[0][0].infoUrl : "",
          ]);
        }
      } else {
        for (var i = 0; i < arguments.length; i++) {
          if (successStatus === arguments[i][1] && arguments[i][0].length > 0) {
            map.set(arguments[i][0][0].name, [
              arguments[i][0][0].info
                ? arguments[i][0][0].info
                : arguments[i][0][0].description
                  ? arguments[i][0][0].description
                  : "",
              arguments[i][0][0].infoUrl ? arguments[i][0][0].infoUrl : "",
            ]);
          }
        }
      }
      ...
      $.each(keys, function(index, key) {
        if (map.has(key)) {
          html +=
            ...
            replaceURL(map.get(key)[0]) +
            "</td><td style='word-break: normal'>" +
            replaceURL(map.get(key)[1], "Link") +
            "</td></tr>";
        }
      });
      ...
      $.each(filters, function(idx, el) {
        ...
        if (vals.length > 0) {
          ...
          if (
            dqtranslation &&
            val in dqtranslation &&
            typeof dqtranslation[val] === "object"
          ) {
            wiki =
              "<a href='https://github.com/AtlasOfLivingAustralia/ala-dataquality/wiki/" +
              dqtranslation[val].name +
          ...
          valuesHtml += "<tr>";
          valuesHtml +=
            '<td class="filter-description" style="word-break: break-word">' +
            replaceURL(description[idx]) +
            "</td>";
          valuesHtml +=
            '<td class="filter-value" style="word-break: normal"><span style="white-space: nowrap;">' +
            el +
            "</span></td>";
  ...
  $("#expandfilters").on("click", function(e) {
    ...
    url = replaceInvalidProfile(
      url,
      $.url().param("qualityProfile"),
  ...
  $(".DQPrefSettingsLink").click(function() {
    ...
    if (!userPref.disableAll) {
      ...
      var userProfileSet =
        userPref.dataProfile != null && userPref.dataProfile.length > 0;
    ...
    } else if (userProfileEnabled && userPref.dataProfile !== null) {
      // if a profile selected and enabled
      profileSelect.val(userPref.dataProfile);
    } else {
      // if no profile selected or selected profile disabled, use system default
  ...
  $("#submitFacets :input.submit").on("click", function(e) {
    ...
    var selectedCount = 0;
    var maxSelected = 15;
    var selectedItemsArray = $("form#facetRefineForm").find(
      ":input.fqs:checked",
    );
    ...
    if (checkedFound && selectedCount > maxSelected) {
      ...
      window.location.href =
        window.location.pathname + BC_CONF.searchString + fqString + hash;
    } else {
  ...
  function setFiltersInitialStatus() {
    ...
    $.each(filterStatus, function(i, el) {
      ...
      if (checked === true) {
        $("#filterRefineForm")
          .find('.expand[data-category="' + category + '"]')
          .show();
        $("#filterRefineForm")
          .find('.expanded[data-category="' + category + '"]')
          .hide();
  ...
  $("#filterRefineForm :button.expand").on("click", function(e) {
    ...
    $("#filterRefineForm")
      .find('.filters[data-category="' + category + '"]')
      .prop("checked", false);
    // hide expand button
    $(this).hide();
  ...
  $("#submitFilters :input.submit").on("click", function(e) {
    ...
    $.each(filterStatus, function(i, status) {
      ...
      if (toDisable) {
        // if to disable, add it to disable list
        ...
        // if to enable, remove it from disable list + remove expanded fqs
        if (disableQualityFilterSet.has(filterlabel)) {
          url = removeFromURL(
            url,
            "disableQualityFilter=" +
  ...
  function appendURL(url, queryParamsToAppend) {
    ...
    if (idx === -1) {
      return url.concat(
        (url.indexOf("?") === -1 ? "?" : "&") + queryParamsToAppend,
      );
    } else {
      return (
        url.slice(0, idx) +
  ...
  function prependURL(url, queryParamsToAppend, afterSearchKey) {
    ...
    if (afterSearchKey) {
      for (; idx < queries.length; idx++) {
        ...
        if (
          !query.startsWith("q=") &&
          !query.startsWith("qid=") &&
          !query.startsWith("lat=") &&
          !query.startsWith("lng=") &&
          !query.startsWith("radius=") &&
          !query.startsWith("taxa=")
        ) {
          break;
        }
      }
  ...
  $("#submitFacets a.wildcard").on("click", function(e) {
    ...
    var link = this;
    var inverseModifier =
      $(link).attr("id").indexOf("exclude") != -1 ? "-" : "";
    var facetName = $("table#fullFacets").data("facet");
    var fqString = "&fq=" + inverseModifier + facetName + ":*";
    window.location.href =
      window.location.pathname + BC_CONF.searchString + fqString;
  });
  ...
  $(
    "a.multipleFacetsLink, a#downloadLink, a#alertsLink, .tooltips, .tooltip, span.dropDown a, div#customiseFacets > a, a.removeLink, .btn, .rawTaxonSumbit",
  ).qtip({
    style: {
  ...
  $("a#alertNewRecords, a#alertNewAnnotations").click(function(e) {
    ...
    var searchParamsEncoded = encodeURIComponent(
      decodeURIComponent(BC_CONF.searchString),
    ); // prevent double encoding of chars
    if (query.length >= 250) {
      url +=
        "queryDisplayName=" +
        encodeURIComponent(query.substring(0, 149) + "...");
    } else {
  ...
  $(".showHideFacetGroup").click(function(e) {
    ...
    group.slideToggle(600, function() {
      if (group.is(":visible")) {
        ...
        if (facets) {
          // dq and custom will be undefined
          for (facet in facets) {
  ...
  $(document).delegate(".thumbImage", "click", function(event) {
    ...
    attribution +=
      "<br>" +
      recordLink +
      "<br><br>If this image is incorrectly<br>identified please flag an<br>issue on the " +
      flagIssueLink +
      "<br>";
    setDialogSize();
  ...
  $("#imageDialog").on("shown.bs.modal", function() {
    ...
    imgvwr.viewImage(
      ...
      undefined,
      {
        ...
        userRatingHelpText: BC_CONF.userRatingHelpText.replace(
          "RECORD_URL",
          recordUrl,
        ),
        savePreferredSpeciesListUrl:
          BC_CONF.savePreferredSpeciesListUrl +
          "?id=" +
          imageId +
          "&scientificName=" +
          scientificName,
  ...
  function getExcludedCounts() {
    ...
    $.get(BC_CONF.excludeCountUrl).done(function(data) {
      ...
      for (var key in data) {
        var categoryEnabled = $(
          ".exclude-count-label[data-category=" + key + "]",
        ).data("enabled");
        data[key] = categoryEnabled
          ? new Intl.NumberFormat().format(parseInt(data[key]))
          : "0";
        $(".exclude-count-label[data-category=" + key + "]")
          .text(data[key])
          .show();
        ...
        if (data[key] === "0") {
          $(".exclude-count-facet[data-category=" + key + "]")
            .text("(" + data[key] + ")")
            .show();
        } else {
          $(".exclude-count-facet[data-category=" + key + "]")
  ...
  if (
    $(".dq-filters-collapse").attr("aria-expanded") === "true" ||
    !$("#group_data_quality").is(":hidden")
  ) {
    getExcludedCounts();
  }
      ...
      $.each(data.occurrences, function(i, el) {
        ...
        var sciNameRawOrMatched =
          el.raw_scientificName === undefined
            ? el.scientificName
            : el.raw_scientificName; //e.g. if data submitted using species ID's instead of scientific names
        ...
        if (el.eventDate)
          detailHtml +=
            br + "Date: " + moment(el.eventDate).format("YYYY-MM-DD");
        if (el.institutionName) {
        ...
        $.each(data, function(i, el) {
          if (el.count > 0) {
            ...
            if (el.level == 0) {
              ...
              dispayName =
                dispayName.charAt(0).toUpperCase() + dispayName.slice(1); // capitalise first letter
            }
            var opt = $(
              "<option value='" +
              el.name +
  ...
  $.fn.ibox = function() {
    ...
    img.each(function() {
      ...
      el.mouseenter(function() {
        ...
        var wh;
        checkwh =
          h < w ? (wh = ((w / h) * resize) / 2) : (wh = (w * resize) / h / 2);
        ...
        if (md.type == "species") {
          ...
          rank =
            "<span style='text-transform: capitalize'>" + md.rank + "</span>: ";
          count = "";
        }

        var itals =
          md.rankId >= 6000 ? "<span style='font-style: italic;'>" : "<span>";
        var infoDiv =
          "<div style=''><a href='" +
          link +
          "' title='" +
...
function loadMoreFacets(facetName, displayName, fsort, foffset) {
  ...
  // Facet value filter
  $("#filterPopupFacet").addClass("hidden");
  $("#filterPopupFacet").val("");
  if (
    facetName == "data_resource_uid" ||
    facetName == "year" ||
    facetName == "occurrence_decade_i"
  ) {
    $("#facet-filter-show").addClass("hidden");
  } else {
    $("#facet-filter-show").removeClass("hidden");
  }
...
function createFacetLinkList(facetResult, queryString, fieldDisplayName) {
  ...
  const linkTitle =
    jQuery.i18n.prop("alatag.filter.results.by") +
    ` ${fieldDisplayName ? fieldDisplayName.toLowerCase() : facetResult.fieldName}`;
  ...
  facetResult.fieldResult.forEach((fieldResult) => {
    if (fieldResult.count > 0) {
      ...
      spanItem.textContent = formatFieldValue(
...
function formatFieldName(fieldName) {
  ...
  if (
    ...
    fieldName.endsWith("_i") ||
    fieldName.endsWith("_d")
  ) {
...
function formatFieldValue(facetName, item) {
  ...
  if (label.indexOf("@") != -1) {
    ...
    var newLabel =
      i18nLabel.indexOf("[") == -1 ? i18nLabel : jQuery.i18n.prop(label);
